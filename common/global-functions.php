...
function get_module_mapping($module) {
    //$urls_mapping = var_get('urls_mapping', NULL);
    $urls_mapping = array(
        'index' => 'index',                         /*首页页面*/
        'help' => 'help',
        'instit-detail' => 'instit/detail',           /*机构详情页面*/
        'activity-detail' => 'activity/detail',       /*活动详情页面*/
        'information-detail' => 'information/detail', /*资讯详情页面*/
        'credit-card-detail' => 'credit-card/detail', /*信用卡详情页面*/
        'house-loan-detail' => 'house-loan/detail',   /*房贷详情页面*/
        'car-loan-detail' => 'car-loan/detail',       /*车贷详情页面*/
        'consume-loan-detail' => 'consume-loan/detail', /*消费贷详情页面*/
        'search' => 'search'
    );

    $url_path = '';
    if (NULL != $urls_mapping) {
        if (array_key_exists($module, $urls_mapping)) {
            $url_path = $urls_mapping[$module];
        }
        else {
            $url_path = $module;
        }
...
function get_login_user() {
    ...
    if (NULL == $user && array_key_exists('geefull_user_cookie', $_COOKIE)) {
        $user_cookie = get_from_cookie('geefull_user_cookie', true, get_crypt_key());
        $user_service = new UserService();

        try {
            $user = $user_service->getUserById((int)$user_cookie);
        }
        catch (BusinessException $ex) {
            handle_exception($ex);
        }

        if (NULL == $user) { //Cannot fetch user, clear cookie.
            put_into_cookie('geefull_user_cookie', '', -1800, false);
            put_into_cookie('geefull_user_mark', '', -1800, false);
        }
    }
...
function get_login_admin_user(){

    if (array_key_exists('geefull_admin_cookie', $_COOKIE)) {
        $userCookie = get_from_cookie('geefull_admin_cookie', true,
                 get_crypt_key());
        ...
        $user = new AdminUser();
        list($user->id, $user->name, $user->role_id, $user->head_pic) = explode("\t", $userCookie);
...
function set_user_to_cookie($user, $expire_time = 0) {
    ...
    if (NULL != $user) {
        put_into_cookie('geefull_user_cookie',
            "$user->id",
            $expire_time, true, get_crypt_key());

        put_into_cookie('geefull_user_mark','1', $expire_time, false, 'geefull_user_mark');
        put_into_cookie('geefull_user_id',$user->id, $expire_time, false);
    }
}
    ...
    try {
        ...
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                if ($page->doValidation()) {
                    $page->doGet();
                    $page->smarty->autoload_filters = array('output' => array('static_url_replace'));
                }
                else {
                    $page->validationFailAction();
                }
...
function get_login_user_id() {
	...
	if (NULL == $user_id && array_key_exists('geefull_user_cookie', $_COOKIE)) {
	    $user_cookie = get_from_cookie('geefull_user_cookie', true, get_crypt_key());
