...
class Common {
    ...
    /**
     * 分析tags，数据库中的tag是按照逗号分隔，显示的时候需要根绝tag格式对每个tag增加链接。
     * @param <type> $tags
     * @param String $format, 显示tag的格式，例如: <a target="_blank" href="search.php?keyword=%s">%s</a>
     */
    public static function splitTags($tags, $format = NULL) {
    	if (NULL == $format) {
    		$format = '<a target="_blank" href="'. var_get('main_url') . 'search.php?keyword=%s"><span style="color:#666666">%s</span></a>';
    	}
        
        $tag_seperators = array(' ', '，', ',','　');

        foreach ($tag_seperators as $value) {
            $tags = str_replace($value, ',', $tags);
        }
        
        $strip_func = 'return ((NULL == $tag || "" == $tag) ? FALSE : TRUE);';
        $tag_array = array_filter(explode(',', $tags), create_function('$tag', $strip_func));

        $tag_str = array();
        foreach ($tag_array as $tag) {
            $tag_str[] = parse_string($format, urlencode($tag), $tag);
        }
   ...
   /**
    ...
    /**
     * 保留当前url的GET参数的添加额外的GET参数
     * @param string $parameter 传入的参数 如'key=value'
     * @return string 保留原有参数再附加额外参数 如'old_key=old_value&key=value'
     */
    public static function urlAddParameter($parameter) {
    $parameters = NULL;
    ...
    $parameters[$add_key] = $add_value;
    foreach ($parameters as $key => $value) {
        if ('' != $value) {
            $result_parameters[] = $key . '=' . $value;
        }
        else {
            $result_parameters[] = $key;
        }
    }
